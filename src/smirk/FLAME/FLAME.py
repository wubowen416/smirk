...
class FLAME(nn.Module):
    ...
    def __init__(
        self,
        flame_model_path="assets/smirk/FLAME2020/generic_model.pkl",
        flame_lmk_embedding_path="assets/smirk/landmark_embedding.npy",
        n_shape=300,
        n_exp=50,
    ):
        ...
        self.register_buffer(
            "l_eyelid",
            torch.from_numpy(np.load(f"assets/smirk/l_eyelid.npy")).to(self.dtype)[
                None
            ],
        )
        self.register_buffer(
            "r_eyelid",
            torch.from_numpy(np.load(f"assets/smirk/r_eyelid.npy")).to(self.dtype)[
                None
            ],
        )
        ...
        lmk_embeddings_mp = np.load(
            "assets/smirk/mediapipe_landmark_embedding/mediapipe_landmark_embedding.npz"
        )
        self.register_buffer(
            "mp_lmk_faces_idx",
    ...
    def forward(
        self, param_dictionary, zero_expression=False, zero_shape=False, zero_pose=False
    ):
        ...
        if zero_expression:
            expression_params = torch.zeros_like(expression_params).to(
                shape_params.device
            )
